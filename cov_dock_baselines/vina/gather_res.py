...
if __name__=='__main__':
    ...
    for i,row in tqdm.tqdm(df.iterrows(),desc='calc metric', total=len(df)):
        pdb_id = row['pdb_id']
        vina_res_filename = args.work_dir + f'/bonded/{pdb_id}/{pdb_id}_ligand_vina_out_5.pdbqt'
        res_pdb_filename = args.work_dir + f'/bonded/{pdb_id}/{pdb_id}_ligand_vina_out_5.pdb'
        reorder_tar_pdb_filename = args.work_dir + f'/bonded/{pdb_id}/{pdb_id}_ligand_reorder_tar.pdb'
        res_filename = args.work_dir + f'/bonded/{pdb_id}/{pdb_id}_ligand_vina_out_5.sdf'
        ...
        if not os.path.exists(vina_res_filename):
            failed_pdb.append(pdb_id)
            rmsd_list.append(float('inf'))
            optimal_rmsd_list.append(float('inf'))
            inter_bond_dist_error_list.append(float('inf'))
            reorder_rmsd_list.append(float('inf'))
        else:
            
            # try:
            #     run(f'obabel -ipdbqt {vina_res_filename} -opdb -O {res_pdb_filename}')
            # except:
            #     failed_pdb.append(pdb_id)
            #     rmsd_list.append(float('inf'))
            #     optimal_rmsd_list.append(float('inf'))
            ...
            # pick the first result of vina output (the best for minimizedAffinity)
            tar_mol = Chem.SDMolSupplier(tar_filename)[0]
            pred_mol = Chem.SDMolSupplier(res_filename)[0]
            if pred_mol == None: # openbabel or autodock output strange molecule
                for i in range(5):
                    pred_mol = Chem.SDMolSupplier(res_filename)[i]
                    if pred_mol !=None:
                        break
            ...
            Chem.MolToPDBFile(pred_mol, res_pdb_filename)
            Chem.MolToPDBFile(tar_mol, reorder_tar_pdb_filename)
            ...
            # optimal_rmsd = optimal_assignment_rmsd(tar_coords,pred_coords)
            # optimal_rmsd_list.append(optimal_rmsd)
            reorder_rmsd_res = reorder_rmsd(res_pdb_filename, reorder_tar_pdb_filename)
            reorder_rmsd_list.append(reorder_rmsd_res)
            # print(f'optimal_rmsd:{optimal_rmsd}; rmsd:{_rmsd}; reorder_rmsd: {reorder_rmsd_res}')
            print(f'rmsd:{_rmsd}; reorder_rmsd: {reorder_rmsd_res}')
    ...
    # print_results(np.array(optimal_rmsd_list), inter_bond_dist_error)
    reorder_res_table = print_results(np.array(reorder_rmsd_list), inter_bond_dist_error)
