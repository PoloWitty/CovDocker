"""
desc:	Get the docking task's lmdb file from infered result of reactive site prediction
author:	Yangzhe Peng
date:	2024/05/21
"""
...
def parse(row, pocket_file, max_len):
    ligand_file = row['bonded ligand']
    pdb_id = row['bonded pocket'].split('/')[-2] # example: ./data/processed/bonded/4WSJ/4WSJ_pocket.pdb
    ...
    # drop this in pipe docking due to this process will drop too many entries and is not necessary because will do pocket drop in unimol task
    ...
    for i in range(ligand_mol.GetNumAtoms()):
        ...
        if atom_name == bond_ligand_atom:
            bond_ligand_idx = i
            break
    assert bond_ligand_idx != None, f"Cannot find the bond in ligand file {ligand_sdf_filename}"
    # get bond pocket idx
    ...
    assert bond_pocket_idx != None, f"Cannot find the bond in pocket file {pocket_file}"
...
def create_selection_query(residue_list):
    query_parts = []
    for x in residue_list:
        resname, chain, resnum = x[0].split(' ') # example: 'ASP A 229'
        resnum = f'`{resnum}`' if resnum.startswith('-') else resnum # process negative resnum
        part = f"(resnum {resnum} and chain {chain} and resname {resname})"
        query_parts.append(part)
    return " or ".join(query_parts)


def get_pkl(row, infer_pocket_mask, res_ids, max_len):
    protein_filename = row['bonded chain']
    pdb = parsePDB(protein_filename)
    # assert pdb.numResidues() == len(infer_pocket_mask)
    name, resName, chainID, resSeq = row['bond'].split('-')[0].split(' ')
    select_res = [res_ids[i] for i,is_pocket in enumerate(infer_pocket_mask) if is_pocket] + [(f"{resName} {chainID} {resSeq}",)] # append the bond residue
    sele_str = create_selection_query(select_res)
    selected_res = pdb.select(sele_str)
    # write to a temporary file
    with NamedTemporaryFile(mode='w+', delete=True) as temp_file:
        temp_filename = temp_file.name +'.pdb'
        writePDB(temp_filename, selected_res)
        res_pkl = parse(row, temp_filename, max_len)
    return res_pkl    

def write_lmdb(dataset_info, infer_res_pkl, outpath="./results", max_len=1022):
    df = pd.read_csv(dataset_info)
    df.set_index('pdb_id', inplace=True)
    infer_data = pickle.load(open(infer_res_pkl, 'rb'))

    
    os.makedirs(outpath, exist_ok=True)
    failed_pdb = []
    outputfilename = os.path.join(outpath, 'test' + ".lmdb")
    try:
    ...
    dataset_idx = 0
    for data in tqdm(infer_data, total=len(infer_data), desc='processing and saving to lmdb'):
        pdb_id = data['pdb_id']
        row = df.loc[pdb_id].squeeze()
        # get_pkl(row, data['protein_mask_target'], data['res_ids'])
        inner_output = get_pkl(row, data['infer_pocket_mask_20A'], data['res_ids'], max_len)
        # inner_output = get_pkl(row, data['infer_pocket_mask'], data['res_ids'], max_len)
...
if __name__=='__main__':
    ...
    parser.add_argument('--infer_res_pkl', type=str, help='The path of the infered result pickle file')
    parser.add_argument("--dataset_info_file", type=str, default="./data/processed/dataset.filtered.random_split.filtered.csv")
    parser.add_argument("--save_dir", type=str, default="./data/processed/dataset/infer_docking/")
    parser.add_argument("--max_len", type=int, default=1024-4) # 1024 - bos - eos - bos - eos
    args = parser.parse_args()
    
    os.makedirs(args.save_dir, exist_ok=True)
    
    write_lmdb(args.dataset_info_file, args.infer_res_pkl, args.save_dir, args.max_len)