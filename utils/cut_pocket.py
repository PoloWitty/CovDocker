...
def gen_pocket(pro, lig, pock_path, cut_off=5):
    ...
    # not used
    # # Calculate the geometric center of the pocket
    # model = cmd.get_model("pock")
    # coords = [atom.coord for atom in model.atom]
    # geometric_center = np.mean(coords, axis=0).tolist()

    # # Create a pseudoatom at the coordinate
    # cmd.pseudoatom("center_pos", pos=geometric_center)
    # # Select residues within a 10Å radius of the geometric center
    # cmd.select("pocket_center_residues", f"byres (pro within {cut_off} of center_pos)")

    # # Save the residues within the 10Å radius of the geometric center
    # cmd.save(pock_path.replace('.pdb', f'{cut_off}Apocket_center_residues.pdb'), "pocket_center_residues and not sol.")
