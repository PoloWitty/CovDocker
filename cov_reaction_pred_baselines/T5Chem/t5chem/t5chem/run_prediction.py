...
from transformers import T5Config, T5ForConditionalGeneration, set_seed
...
def predict(args):
    ...
    if isinstance(predictions[0], list):
        ...
        for i in range(1, args.num_preds+1):
            ...
            res_dict[f'test_molecular_accuracy_top_{i}'] = correct/len(test_df)
        if args.use_wandb:
            import wandb
            wandb.init(
                project="res_reaction",
                config = vars(torch.load(open(args.train_config_filename,'rb')))
            )
