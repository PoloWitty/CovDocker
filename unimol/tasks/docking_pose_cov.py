...
@register_task("docking_pose_cov")
class DockingPoseCov(UnicoreTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--min-dist-norm-weight",
            type=float,
            default=1.0,
            help="weight for the inter bond is min dist norm loss"
        )
        parser.add_argument(
            "--min-dist-norm-type",
            type=int,
            default=1,
            help="0: use (d_ij-min(D_inter)).abs(), 1: use ReLU(d_ij - D_inter) type norm, 2: use CE(d, one_hot[i]) type norm"
        )
    ...
    def load_dataset(self, split, **kwargs):
        ...
        # dataset = RemoveHydrogenPocketDataset(
        #     dataset, "atoms", "coordinates", "holo_coordinates", None, False, False
        # )

        orig2cropped_dataset = KeyDataset(dataset, "orig2cropped")
        orig2cropped_dataset = FromNumpyDataset(orig2cropped_dataset)
        
        inter_bond_dataset = KeyDataset(dataset, "inter_bond")
        inter_bond_dataset = FromNumpyDataset(inter_bond_dataset)
        ...
        src_dataset = TokenizeDataset(
            src_dataset, self.dictionary, max_seq_len=self.args.mol_max_seq_len
        )
        ...
        src_pocket_dataset = TokenizeDataset(
            ...
            max_seq_len=self.args.pocket_max_seq_len,
        )
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                ...
                "orig2cropped_pocket": RightPadDataset(orig2cropped_dataset, pad_idx=-1),
                "inter_bond": RightPadDataset(inter_bond_dataset, pad_idx=-1),
            },
        )
