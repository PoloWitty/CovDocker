...
# from unimol_docking_v2/unimol/data/cropping_dataset.py
class CroppingPocketDataset_dockingV2(BaseWrapperDataset):
...
class CroppingPocketDockingPoseDataset(BaseWrapperDataset):
    def __init__(
        self, dataset, seed, atoms, coordinates, holo_coordinates, orig2cropped='orig2cropped', inter_bond="inter_bond", max_atoms=256
    ):
        self.dataset = dataset
        self.seed = seed
        self.atoms = atoms
        ...
        self.holo_coordinates = holo_coordinates
        self.max_atoms = max_atoms
        self.orig2cropped = orig2cropped
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        coordinates = dd[self.coordinates]
        holo_coordinates = dd[self.holo_coordinates]
        inter_bond = dd[self.inter_bond]
        orig2cropped = np.full((len(atoms),),-1, dtype=np.int32)
        ...
        if self.max_atoms and len(atoms) > self.max_atoms:
            with data_utils.numpy_seed(self.seed, epoch):
                ...
                if inter_bond[0] not in index:
                    index = np.concatenate((index, [inter_bond[0]])) # never drop inter-bond pocket atom
                assert inter_bond[0] in index
            ...
            # no crop
            orig2cropped = np.arange(len(atoms))

        dd[self.atoms] = atoms
        dd[self.coordinates] = coordinates.astype(np.float32)
        dd[self.holo_coordinates] = holo_coordinates.astype(np.float32)
        dd[self.orig2cropped] = orig2cropped.astype(np.int32)
        return dd
