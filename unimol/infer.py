...
def main(args):
    ...
    json.dump(namespace_to_dict(args),open(os.path.join(args.results_path,'infer_args.json'), 'w'), indent=4)
    ...
    for subset in args.valid_subset.split(","):
        ...
        time_cost = {
            'Total infer time cost': sum(time_cost_list), 
            'Average infer time cost per sample': sum(time_cost_list) / len(dataset)
        }
        logger.info(str(time_cost))
        json.dump(time_cost,open(os.path.join(args.results_path,'infer_time.json'), 'w'), indent=4)
    return None
