...
class TokenClassificationCrossEntropyPocketLoss(FinetuneCrossEntropyLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if not self.training:
            ...
            infer_pocket_mask_20A = dis < 20 # for pipeline blind docking
        ...
        if not self.training:
            ...
            infer_pocket_mask_20A = infer_pocket_mask_20A.data[is_protein_mask]
            res_ids = sample['res_ids']
            assert infer_pocket_mask.shape[0] == len(res_ids)
            assert infer_pocket_mask_20A.shape[0] == len(res_ids)
            logging_output = {
                ...
                "infer_pocket_mask_20A": infer_pocket_mask_20A.data.detach().cpu(),
