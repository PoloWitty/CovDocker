...
reactive_site_run_name="ours_seed2"
results_path="./save/reactive_site/${reactive_site_run_name}/infer"  # replace to your results path
lmdb_path="${results_path}/unseen_inferred_pocket"
mkdir -p $lmdb_path

# prepare input from inferred pocket
python utils/infer_pocket_pdb.py --infer_res_pkl "${results_path}/${reactive_site_run_name}_unseen.out.pkl" --dataset_info_file ./data/processed/dataset.filtered.unseen.csv --save_dir $lmdb_path



# infer on unseen set
docking_run_name="ours_seed2"
...
results_path="./save/dock/${docking_run_name}/unseen_pipe_infer"  # replace to your results path
...
CUDA_VISIBLE_DEVICES=1 python ./unimol/infer.py --user-dir ./unimol $data_path --valid-subset test \
...
run_name="site20A_${reactive_site_run_name}_dock_${docking_run_name}"
export WANDB_RUN_ID="unseen_pipe_post10sigma_${run_name}_eval${time}"
