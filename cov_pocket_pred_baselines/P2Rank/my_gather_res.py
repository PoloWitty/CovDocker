"""
desc:	Compute DCC metrics for pocket prediction of p2rank
author:	Yangzhe Peng
date:	2024/05/12
"""
...
if __name__=='__main__':
    ...
    parser.add_argument("--run-seed", type=int, default=0, help='the seed used when run p2rank')
    parser.add_argument("--run-thread", type=int, default=10, help='the threads num used when run p2rank')
    ...
    for i,row in tqdm.tqdm(split_df.iterrows(),total=len(split_df),desc='processing'):
        ...
        # get p2rank predict pocket center
        pred_res_filename = f'{data_dir}/{pdb_id}_chain_within_10A.pdb_predictions.csv'
        d = pd.read_csv(pred_res_filename)
        d.columns = d.columns.str.strip()
        try:
            rank1_center = d.iloc[0][['center_x', 'center_y', 'center_z']].values.astype(float)
